...
export const useDashboardStore = create<DashboardState>((set, get) => ({
  ...
  refreshData: async () => {
    ...
    try {
      const proxyStatusPromise = ApiService.proxy.getStatus().catch(() => null)
      const statisticsPromise = ApiService.statistics.getToday().catch(() => null)
      const persistentStatsPromise = ApiService.statistics.get().catch(() => null)
      const providersPromise = ApiService.providers.getAll().catch(() => [])
      const accountsPromise = ApiService.accounts.getAll().catch(() => [])
      const providerStatusPromise = ApiService.providers.checkAllStatus().catch(() => ({}))
      const logsPromise = ApiService.logs.get({ limit: 10 }).catch(() => [])
      const trendPromise = ApiService.logs.getTrend(7).catch(() => [])
      const requestLogTrendPromise = ApiService.requestLogs.getTrend(7).catch(() => [])
      ...
      ]) as [ProxyStatus | null, any | null, any, Provider[], Account[], Record<string, ProviderCheckResult>, LogEntry[], LogTrend[], any[]]
      ...
      const requestLogsPromise = ApiService.requestLogs.get({ limit: 100 }).catch(() => [])
