...
export class GLMAdapter extends BaseOAuthAdapter {
  ...
  async startLogin(options: OAuthOptions): Promise<OAuthResult> {
    ...
    try {
      this.emitProgress('pending', 'Please log in via browser and enter Refresh Token manually')
      
      return {
        success: false,
        providerId: options.providerId,
        providerType: 'glm',
        error: 'Please log in via browser, extract Refresh Token from Developer Tools and enter manually. URL: ' + GLM_API_BASE,
      }
    } catch (error) {
      console.error('[GLM] startLogin error:', error)
