...
export async function getDeepSeekHash(): Promise<DeepSeekHash> {
  if (!deepSeekHashInstance) {
    ...
    // Resolve WASM path based on environment
    // When bundled with Vite, it's relative to __dirname
    // When running from source, it's in the project root
    let wasmPath = ''
    
    if (process.env.NODE_ENV === 'production') {
      // Production path
      wasmPath = path.join(__dirname, 'sha3_wasm_bg.7b9ca65ddd.wasm')
    } else {
      // Development path
      wasmPath = path.resolve(process.cwd(), 'sha3_wasm_bg.7b9ca65ddd.wasm')
    }
